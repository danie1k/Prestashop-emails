					...
					foreach ($order->product_list as $key => $product)
					{
						...
						if (isset($customized_datas[$product['id_product']][$product['id_product_attribute']]))
						{
							...
							'<tr style="background-color: '.($key % 2 ? '#DDE2E6' : '#EBECEE').';">
								...
								<td><strong>'.$product['name'].(isset($product['attributes']) ? ' - '.$product['attributes'] : '').' - '.Tools::displayError('Customized').(!empty($customization_text) ? ' - '.$customization_text : '').'</strong></td>
						...
						if (!$customization_quantity || (int)$product['cart_quantity'] > $customization_quantity)
							...
							'<tr style="background-color: '.($key % 2 ? '#DDE2E6' : '#EBECEE').';">
								<td>'.$product['reference'].'</td>
								<td><strong>'.$product['name'].(isset($product['attributes']) ? ' - '.$product['attributes'] : '').'</strong></td>
